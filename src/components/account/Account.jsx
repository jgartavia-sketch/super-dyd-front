...
import { Copy, Eye, EyeOff, Gift, LogIn, LogOut, Share2, Star, UserPlus, Users } from "lucide-react";
import { accountApi } from "../../services/api";

const emptyRegister = { name: "", email: "", phone: "", password: "", confirmPassword: "", referral_code: "" };
...
export default function Account() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const data = mode === "register" ? await accountApi.register(registerData) : await accountApi.login({ email: form.email, password: form.password });
  ...
  const changeMode = (nextMode) => { setMode(nextMode); setMessage(""); setVisiblePasswords({ password: false, confirmPassword: false }); };
  ...
  return <section className="account-section"><div className="account-heading"><span className="section-kicker">Mi cuenta</span><h2>Puntos que conectan</h2><p>Registrate, recibí tu código personal y ganá beneficios al compartir D&amp;D.</p></div><div className="auth-shell"><div className="auth-benefits"><Gift size={38} /><h3>Tu comunidad también suma</h3><p>Compartí tu enlace único. Cuando una persona se registre correctamente con él, el sistema lo asociará a tu cuenta.</p><div><span><Star size={19} />Consultá tus puntos</span><span><Users size={19} />Revisá tus referidos</span><span><Share2 size={19} />Compartí por WhatsApp</span></div></div><form className="auth-form" onSubmit={submit}><div className="auth-tabs"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => changeMode("login")}><LogIn size={17} />Ingresar</button><button type="button" className={mode === "register" ? "active" : ""} onClick={() => changeMode("register")}><UserPlus size={17} />Crear cuenta</button></div>{mode === "register" && <><label>Nombre completo<input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>WhatsApp<input required inputMode="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label></>}<label>Correo electrónico<input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label>Contraseña<div className="password-field"><input required type={visiblePasswords.password ? "text" : "password"} minLength="8" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button type="button" aria-label={visiblePasswords.password ? "Ocultar contraseña" : "Ver contraseña"} onClick={() => togglePassword("password")}>{visiblePasswords.password ? <EyeOff size={20} /> : <Eye size={20} />}</button></div></label>{mode === "register" && <><label>Confirmar contraseña<div className="password-field"><input required type={visiblePasswords.confirmPassword ? "text" : "password"} minLength="8" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} /><button type="button" aria-label={visiblePasswords.confirmPassword ? "Ocultar contraseña" : "Ver contraseña"} onClick={() => togglePassword("confirmPassword")}>{visiblePasswords.confirmPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button></div></label><label>Código de referido <small>(opcional)</small><input value={form.referral_code} onChange={(e) => setForm({ ...form, referral_code: e.target.value.trim().toUpperCase() })} /></label></>}{message && <p className="form-message">{message}</p>}<button className="primary-btn submit-btn" disabled={loading}>{loading ? "Procesando…" : mode === "login" ? "Ingresar a mi cuenta" : "Crear mi cuenta"}</button></form></div></section>;
}
